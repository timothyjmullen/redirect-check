...
function validate(str) {
    if (/(\s|\+|\%20)/ig.test(str)) {
        str = str.replace(/(\s|\+|\%20)/ig, '<span class="warn warning" id="time">$1<span class="warn tooltip" content="Spaces in image names can cause loading problems in Gmail."></span></span>');
    }
    return str;
}
...
function validateSize(num, formatted) {
    if (num > 1048576) {
        return '<span class="warn warning" id="time">' + formatted + '<span class="warn tooltip" content="This image is larger than recommended for email; it could load very slowly on mobile devices."></span></span>'
    }
    return formatted;
}
